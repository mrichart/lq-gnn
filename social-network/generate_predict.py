...
def parse_args():
    ...
    parser.add_argument("--evaluation_dir", type=Path, default=Path("data/evaluation"), help="Directory where to store the prediction dataset.")
    parser.add_argument("--input_graph_dir", type=Path, default=Path("/home/matias/devel/uqsim-power-management-beta/architecture/social_network/json"), help="Directory containing input graphs.")
...
def generate_graphs(input_graph_dir, input_dir, output_dir, rejection_threshold):
        ...
        with open(os.path.join(input_dir, filename), "r") as file:
            for line in file:
                ...
                if line.startswith("total:"):
                    ...
                    total_avg_rt = int(float(line_data[1])*1000)
                    total_p95_rt= int(float(line_data[3])*1000)
                    total_p99_rt = int(float(line_data[4])*1000)
                    #if total_avg_rt > 10:
                    #   complete_log = False
                if line.startswith("path0:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[0] = int(float(line_data[1])*1000)
                    path_p95_rt[0] = int(float(line_data[3])*1000)
                    path_p99_rt[0] = int(float(line_data[4])*1000)
                if line.startswith("path1:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[1] = int(float(line_data[1])*1000)
                    path_p95_rt[1] = int(float(line_data[3])*1000)
                    path_p99_rt[1] = int(float(line_data[4])*1000)
                if line.startswith("path2:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[2] = int(float(line_data[1])*1000)
                    path_p95_rt[2] = int(float(line_data[3])*1000)
                    path_p99_rt[2] = int(float(line_data[4])*1000)
                ...
                if reached_micro_stats:
                    ...
                    if not re.search(r'\d', line_data[0]):
                        micro_cpu[line_data[0]] = int(float(line_data[1])*100)
                        micro_requests[line_data[0]] = float(line_data[2])
                        micro_avg_rt[line_data[0]] = int(float(line_data[3])*1000)
                        micro_p95_rt[line_data[0]] = int(float(line_data[7])*1000)
                        micro_p99_rt[line_data[0]] = int(float(line_data[8])*1000)
                    

        #load in requests per second
        load = conf_map['kqps']*1000
...
if __name__ == "__main__":
    ...
    #     os.makedirs(_dir, exist_ok=True)
    generate_graphs(args.input_graph_dir, args.input_dir, args.output_dir, args.rejection_threshold)
    join_graphs_into_dataset(list(Path(args.output_dir).glob("*.json")), args.evaluation_dir, empty_dirs=True)
