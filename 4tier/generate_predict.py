...
import argparse
import numpy as np
from  graph_path_2 import generate_graph
...
def parse_args():
    ...
    parser.add_argument("--input_graph_dir", type=Path, default=Path("/home/matias/devel/uqsim-power-management-beta/architecture/4tier_nginx_rpc/json"), help="Directory containing input graphs.")
...
def generate_graphs(input_graph_dir, input_dir, output_dir, rejection_threshold):
    ...
    for filename in os.listdir(input_dir):
        ...
        latency_2_3 = conf_map["latencyfogcloud"]
        machine_latencies = {0: {0: 0, 1: 0, 2: latency_0_2, 3: np.inf}, 1: {0: 0, 1: 0, 2: latency_1_2, 3: np.inf}, 2: {0: latency_0_2, 1: latency_1_2, 2: 0, 3:latency_2_3}, 3: {0: np.inf, 1: np.inf, 2: latency_2_3, 3: 0}}
        valid = (accepted_requests / (load*1000)) > (1-rejection_threshold)
        if complete_log and len(path_avg_rt) == 3:
            G = generate_graph(input_graph_dir, load=load, threads=threads, replicas=replicas, cores=cores, machine_latencies=machine_latencies, avg_rt=path_avg_rt, p95_rt=path_p95_rt, p99_rt=path_p99_rt, deployment=deployment, valid=valid, total_avg_rt=total_avg_rt, total_p95_rt=total_p95_rt, total_p99_rt=total_p99_rt)
