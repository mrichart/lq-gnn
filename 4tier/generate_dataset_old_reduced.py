...
def parse_args():
    ...
    parser.add_argument("--keep_ratio", type=float, default=1.0, help="Ratio of files to skip.")
...
def generate_graphs(input_graph_dir, input_dir, output_dir, output_valid_dir, rejection_threshold, keep_ratio):
    #print("Generating graphs")
    file_num = 0
    for filename in os.listdir(input_dir):
        if np.random.uniform(0, 1) > keep_ratio:
            continue
            ...
            for line in file:
                ...
                if reached_micro_stats:
                    ...
                    if not re.search(r'\d', line_data[0]):
                        ...
                        micro_avg_rt[line_data[0]] = int(float(line_data[5])*1000)
                        micro_p95_rt[line_data[0]] = int(float(line_data[6])*1000)
                        micro_p99_rt[line_data[0]] = int(float(line_data[7])*1000)
        ...
        latency_0_2 = latency_0_2*1000
        latency_1_2 = latency_0_2
        latency_2_4 = conf_map["latencyfogcloud"]*1000
...
if __name__ == "__main__":
    ...
    for _dir in [args.output_valid_dir, args.train_valid_dir, args.validation_valid_dir, args.test_valid_dir, Path(args.output_valid_dir) / "train", Path(args.output_valid_dir) / "validation", Path(args.output_valid_dir) / "test"]:
        os.makedirs(_dir, exist_ok=True)
    generate_graphs(args.input_graph_dir, args.input_dir, args.output_dir, args.output_valid_dir, args.rejection_threshold, args.keep_ratio)
