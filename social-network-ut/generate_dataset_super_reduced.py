...
def parse_args():
    ...
    parser.add_argument("--input_graph_dir", type=Path, default=Path("~/uqsim-power-management-beta/architecture/social_network_user_tl/json"), help="Directory containing input graphs.")
    ...
    for filename in os.listdir(input_dir):
        ...
        with open(os.path.join(input_dir, filename), "r") as file:
            for line in file:
                ...
                if line.startswith("path3:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[3] = float(line_data[1])*1000
                    path_p95_rt[3] = float(line_data[3])*1000
                    path_p99_rt[3] = float(line_data[4])*1000
                if line.startswith("path4:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[4] = float(line_data[1])*1000
                    path_p95_rt[4] = float(line_data[3])*1000
                    path_p99_rt[4] = float(line_data[4])*1000
                if line.startswith("path5:"):
                    line_data = re.split(';|:',line.strip())
                    path_avg_rt[5] = float(line_data[1])*1000
                    path_p95_rt[5] = float(line_data[3])*1000
                    path_p99_rt[5] = float(line_data[4])*1000
        ...
        deployment = conf_map['depl']
        threads = {"nginx": conf_map['ngx'], "user_timeline": conf_map['ut'], "user_timeline_redis": conf_map['utRedis'], "user_timeline_mongodb": conf_map['utMongo'], "user_timeline_mongo_io": conf_map['utMongoIO'], "post_storage": conf_map['ps'], "post_storage_memcached": conf_map['psMmc'], "post_storage_mongodb": conf_map['psMongo'], "post_storage_mongo_io": conf_map['psMongoIo']}
        replicas = {"nginx": 1, "user_timeline": 1, "user_timeline_redis": 1, "user_timeline_mongodb": 1, "user_timeline_mongo_io": 1, "post_storage": 1, "post_storage_memcached": 1, "post_storage_mongodb": 1, "post_storage_mongo_io": 1}
        cores = {"nginx": conf_map['ngxCor'], "user_timeline": conf_map['utCor'], "user_timeline_redis": conf_map['utRedisCor'], "user_timeline_mongodb": conf_map['utMongoCor'], "user_timeline_mongo_io": conf_map['utMongoIOCor'], "post_storage": conf_map['psCor'], "post_storage_memcached": conf_map['psMmcCor'], "post_storage_mongodb": conf_map['psMongoCor'], "post_storage_mongo_io": conf_map['psMongoIOCor']}
        
        latency_1_3 = conf_map["ledgecloud"]*1000
        latency_1_5 = conf_map["ledgefog"]*1000
        latency_5_7 = conf_map["lfogcloud"]*1000
        machine_latencies = {0: {0: 0, 1: 0, 2: np.inf, 3: np.inf, 4: np.inf, 5: np.inf, 6:np.inf, 7:np.inf, 8:np.inf}, 1: {0: 0, 1: 0, 2: 0, 3: latency_1_3, 4: np.inf, 5: latency_1_5, 6:np.inf, 7:np.inf, 8:np.inf}, 2: {0: np.inf, 1: 0, 2: 0, 3: np.inf, 4: np.inf, 5:np.inf, 6:np.inf, 7:np.inf, 8:np.inf}, 3: {0: np.inf, 1: latency_1_3, 2: np.inf, 3: 0, 4: 0, 5: np.inf, 6:np.inf, 7:np.inf, 8:np.inf}, 4:{0: np.inf, 1: np.inf, 2: np.inf, 3: 0, 4: 0, 5: np.inf, 6:np.inf, 7:np.inf, 8:np.inf}, 5:{0: np.inf, 1: latency_1_5, 2: np.inf, 3: np.inf, 4: np.inf, 5: 0, 6:0, 7:latency_5_7, 8:np.inf}, 6:{0: np.inf, 1: np.inf, 2: np.inf, 3: np.inf, 4: np.inf, 5: 0, 6:0, 7:np.inf, 8:np.inf}, 7:{0: np.inf, 1: np.inf, 2: np.inf, 3: np.inf, 4: np.inf, 5: latency_5_7, 6:np.inf, 7:0, 8:0}, 8:{0: np.inf, 1: np.inf, 2: np.inf, 3: np.inf, 4: np.inf, 5: np.inf, 6:np.inf, 7:0, 8:0}}
        ####################################

        valid = (accepted_requests / (load)) > (1-rejection_threshold)

        if complete_log and len(path_avg_rt) == 6:
